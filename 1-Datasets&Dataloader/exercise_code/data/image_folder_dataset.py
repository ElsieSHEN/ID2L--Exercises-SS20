...
class ImageFolderDataset(Dataset):
    ...
    def make_dataset(directory, class_to_idx):
        ...
        for cls, label in sorted(class_to_idx.items()):
    ...
    def __getitem__(self, index):
        ...
        image_np = self.load_image_as_numpy(image_path)
...
def compute_image_mean_and_std(images):
    ...
    dim = images.shape # dim = (N, H, W, C)
    channels = images.reshape((dim[0]*dim[1]*dim[2]), dim[3]) # (N*H*W, C)

    mean = np.asarray([channels[:,i].mean() for i in range(dim[3])])
    std = np.asarray([channels[:,i].std() for i in range(dim[3])])
    ...
    def __call__(self, images):
        ...
        for image in images:
            for i in range(len(image)):
                image[i] = (image[i]-self._data_min) / (self._data_max-self._data_min) * (self.max-self.min) + self.min
